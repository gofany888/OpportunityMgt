...
*   **Header 轻量结构**：通过 Teleport 注入全局 Header 的页面，优先采用“标题 + 紧邻副标题 + 最右状态徽章”的轻量结构。标题区必须支持 `flex: 1` 拉伸，避免右侧信息只能在局部容器中对齐。
*   **Header 识别元素克制化**：Header 中的识别图形优先使用细竖条、小图标、轻徽章，不使用过大的重色图标块抢占标题权重。
