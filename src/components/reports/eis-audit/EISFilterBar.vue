<template>
  <div class="eis-filter-bar">
    <!-- 顶部：搜索和状态过滤 -->
    <div class="eis-filter-bar__top">
      <div class="flex items-center gap-6 flex-1 max-w-[1600px] mx-auto">
        <!-- 全局搜索 -->
        <a-input 
          v-model:value="searchQuery" 
          placeholder="全局搜索商机名、EIS ID 或 负责人..." 
          class="!rounded-full !bg-gray-50 !border-transparent hover:!border-blue-400 focus:!border-blue-500 !px-4 !py-1.5 w-[360px]"
        >
          <template #prefix>
        ...
        <div class="flex items-center gap-3">
          <span class="text-[12px] text-gray-400 font-medium">对账状态过滤:</span>
          ...
          <a-popover v-model:open="popoverVisible" trigger="click" placement="bottomRight" overlayClassName="filter-popover">
            <template #content>
              <div class="flex flex-col gap-4 w-[280px] p-2">
                <div>
                  <div class="text-[14px] text-gray-500 mb-2 font-medium">按部门过滤</div>
                  <a-select v-model:value="tempDept" placeholder="请选择部门" class="w-full text-[14px]" allowClear :getPopupContainer="triggerNode => triggerNode.parentNode">
                    <a-select-option value="云业务交付部">云业务交付部</a-select-option>
                    <a-select-option value="金融业务部">金融业务部</a-select-option>
                  </a-select>
                </div>
                <div>
                  <div class="text-[14px] text-gray-500 mb-2 font-medium">按金额过滤</div>
                  <a-select v-model:value="tempAmount" placeholder="请选择金额范围" class="w-full text-[14px]" allowClear :getPopupContainer="triggerNode => triggerNode.parentNode">
    ...
    <!-- 底部：已选条件 (有条件时才显示) -->
    <div class="eis-filter-bar__bottom" v-if="hasFilters">
      <div class="flex flex-wrap items-center gap-3 w-full max-w-[1600px] mx-auto">
        <span class="text-[14px] text-gray-400 font-medium">当前筛选口径:</span>
        
        <a-tag v-if="activeDept" closable @close="activeDept = null; checkFilters()" class="!rounded-full !border-none !text-[12px] px-3 py-1 bg-[#4068f6] text-white flex items-center font-semibold tracking-wide shadow-sm">
            部门: {{ activeDept }}
            <template #closeIcon><close-outlined class="text-white hover:text-gray-200 opacity-80 text-[10px] ml-1.5" /></template>
        </a-tag>
        
        <a-tag v-if="activeAmount" closable @close="activeAmount = null; checkFilters()" class="!rounded-full !border-none !text-[12px] px-3 py-1 bg-[#4068f6] text-white flex items-center font-semibold tracking-wide shadow-sm">
            金额: {{ activeAmount }}
            <template #closeIcon><close-outlined class="text-white hover:text-gray-200 opacity-80 text-[10px] ml-1.5" /></template>
        </a-tag>

        <!-- 重置全部过滤 (红色斜体) -->
        <a-button type="link" class="!text-[14px] !p-0 ml-2 shadow-none border-none h-auto text-gray-400 hover:text-[#1677ff]" @click="resetAll">
            <span>重置全部过滤</span>
        </a-button>
      </div>
    </div>
